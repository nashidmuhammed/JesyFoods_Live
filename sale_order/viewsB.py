...
def payments_new(request):
    if request.method == 'POST':
        ...
        if group == 'all':
            ...
            sale = Sale.objects.filter(party_name__party_group=group,t_type='payment', date__date__gte=sd, date__date__lte=ed).order_by('-date')
...
def party_view(request):
    try:
        ...
        if request.method == 'POST':
            ...
            party.party_group = request.POST['party_group']
        ...
        #end pagination
        data = {'party': party, 'sale': page_obj}
        if request.user.groups.filter(name="admin").exists():
            ...
            data = {'party': party, 'sale': page_obj, 'today': today}
...
def home(request):
    try:
        ...
        if request.user.groups.filter(name="admin").exists():
            ...
            total = Sale.objects.filter(user=request.user, t_type='payment', date__date__gte=timezone.now().date()).aggregate(received_sum=Sum('received'))['received_sum'] or 0
            ...
            this_month = Sale.objects.filter(party_name__party_group='fasal', t_type='payment', date__date__gte=t_sd,date__date__lte=t_ed).aggregate(received_sum=Sum('received'))['received_sum'] or 0
            prev_month_coll = Sale.objects.filter(party_name__party_group='fasal', t_type='payment', date__date__gte=p_sd,date__date__lte=p_ed).aggregate(received_sum=Sum('received'))['received_sum'] or 0
        ...
        if request.method == 'POST':
            ...
            if type == 'filter':
                ...
                if filt == 'Prev Month':
                    ...
                    sale = Sale.objects.filter(t_type='sale', date__date__range=[sd, ed]).order_by('-date')
                    rtn = Sale.objects.filter(t_type='return', date__date__range=[sd, ed]).order_by('-date')
